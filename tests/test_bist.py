...
class BISTDUT(Module):
    def __init__(
        ...
        self.submodules.pattern_mem = PatternMemory(
            data_width, pattern_mem_length, addr_width=address_width, pattern_init=pattern_init
        )
        ...
        if reader:
            self.read_port = LiteDRAMNativePort(
                address_width=address_width, data_width=data_width, mode="read"
            )
            self.submodules.reader = Reader(self.read_port, self.pattern_mem, **inverter_kwargs)
            self.reader.add_csrs()

        if writer:
            self.write_port = LiteDRAMNativePort(
                address_width=address_width, data_width=data_width, mode="write"
            )
...
def access_pattern_test(bist_name, mem_inc, pattern, count):
    ...
    def test(self):
        ...
        dut = BISTDUT(
            pattern_init=pattern, writer="writer" in bist_name, reader="reader" in bist_name
        )
        ...
        if mem_inc:
            expected = [
                (count + addr, we, data) for count, (addr, data) in zip(range(count), pattern_cycle)
            ]
        else:
...
class TestWriter(unittest.TestCase):
    ...
    test_mem_inc_pattern_noinc = access_pattern_test(
        "writer", mem_inc=True, pattern=PATTERNS_ADDR_0, count=13
    )
    test_mem_inc_pattern_inc = access_pattern_test(
        "writer", mem_inc=True, pattern=PATTERNS_ADDR_INC, count=13
    )
    test_mem_noinc_pattern_noinc = access_pattern_test(
        "writer", mem_inc=False, pattern=PATTERNS_ADDR_0, count=13
    )
    test_mem_noinc_pattern_inc = access_pattern_test(
    ...
    def test_row_data_invertion(self):
        ...
        for addr, we, data in dut.commands:
            ...
            expected = (
                0x55555555555555555555555555555555 if invert else 0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            )
    ...
    def test_multiple_addresses(self):
        ...
        # fill whole memory with the same data
        pattern = [
            (0x00, 0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA),
...
class TestReader(unittest.TestCase):
    # Verify correctness of access pattern depending on pattern addresses and mem_mask
    test_mem_inc_pattern_noinc = access_pattern_test(
        "reader", mem_inc=True, pattern=PATTERNS_ADDR_0, count=13
    )
    test_mem_inc_pattern_inc = access_pattern_test(
        "reader", mem_inc=True, pattern=PATTERNS_ADDR_INC, count=13
    )
    test_mem_noinc_pattern_noinc = access_pattern_test(
        "reader", mem_inc=False, pattern=PATTERNS_ADDR_0, count=13
    )
    test_mem_noinc_pattern_inc = access_pattern_test(
        "reader", mem_inc=False, pattern=PATTERNS_ADDR_INC, count=13
    )
    ...
    def test_error_detection(self):
        ...
        def generator(dut):
            ...
            self.assertEqual(
                (yield from dut.reader._error_ready.read()), 0, msg="FIFO not empty after reset"
            )
            self.assertEqual(
                (yield from dut.reader._ready.read()), 1, msg="Reader not ready after reset"
            )
            ...
            for error in errors:
                ...
                self.assertEqual(
                    (yield from dut.reader._error_data.read()), 0xFFFFFFFFFFFFFFFEFFFFFFFFFFFFFFFF
                )
                self.assertEqual(
                    (yield from dut.reader._error_expected.read()),
    ...
    def test_row_hammer_attack_pattern(self):
        ...
        def generator(dut):
            ...
            assert (
                len(row_addresses) & (len(row_addresses) - 1) == 0
            ), "Pattern length must be power of 2"
        ...
        def generator(dut):
            ...
            for error in errors:
                ...
                if inversion_address_matcher(error >> row_shift, modulo_divisor, selection_mask):
                    self.assertEqual(
                        (yield from dut.reader._error_data.read()),
                        0x5D555555555555555555555555555555,
                    )
                    self.assertEqual(
                        (yield from dut.reader._error_expected.read()),
                    ...
                    self.assertEqual(
                        (yield from dut.reader._error_data.read()),
                        0xABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA,
                    )
                    self.assertEqual(
                        (yield from dut.reader._error_expected.read()),
        ...
        for addr, we, data in dut.commands:
            ...
            if addr in errors:
                expected = (
                    0x5D555555555555555555555555555555
                    if invert
                    else 0xABAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                )
            else:
                expected = (
                    0x55555555555555555555555555555555
                    if invert
                    else 0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                )
            self.assertEqual(data, expected, msg=addr)
