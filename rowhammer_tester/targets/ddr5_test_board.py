...
class AsyncFIFOWraperDUT(Module, AutoCSR):
    def __init__(self, i_cd_name, o_cd_name):
        ...
        self.comb += [
            If(self.start_csr.storage, we.eq(1)),
            rst.eq(self.rst_csr.storage),
        ]
...
class CRG(Module):
    def __init__(self, platform, sys_clk_freq, iodelay_clk_freq):
        ...
        mmcm.create_clkout(
            self.cd_sys4x_raw, 4 * sys_clk_freq, buf=None, with_reset=False, platform=platform
        )
        mmcm.create_clkout(
...
def main():
    parser = common.ArgumentParser(
        description="LiteX SoC on DDR5 Test Board", sys_clk_freq="200e6", module="MT60B2G8HB48B"
    )
    ...
    soc.platform.toolchain.pre_synthesis_commands.append(
        "set_property -name "
        "{{STEPS.OPT_DESIGN.ARGS.MORE OPTIONS}} "
