...
from litedram.common import GeomSettings, PhySettings, TimingSettings
from litedram.core.controller import ControllerSettings
from litedram.frontend.dma import LiteDRAMDMAReader
from litedram.gen import LiteDRAMCoreControl, get_dram_ios
...
class RowHammerSoC(SoCCore):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.submodules.leds = LedChaser(
            pads=self.platform.request_all("user_led"), sys_clk_freq=sys_clk_freq
        )
        self.add_csr("leds")
        ...
        if sdram_module_spd_file is not None:
            self.logger.info(
                "Using DRAM module {} data: {}".format(colorer("SPD"), sdram_module_spd_file)
            )
        ...
        if not args.sim:
            ...
            self.submodules.etherbone = LiteEthEtherbone(
                self.ethcore.udp, self.udp_port, mode="master"
            )
        ...
        if not args.no_memory_bist:
            ...
            self.add_memory(
                self.writer_pattern_mem.data, name="writer_pattern_data", origin=0x20000000
            )
            self.add_memory(
                self.writer_pattern_mem.addr, name="writer_pattern_addr", origin=0x21000000
            )
            self.logger.info(
                "{}: Length: {}, Data Width: {}-bit, Address width: {}-bit".format(
                    colorer("Writer BIST pattern"),
                    colorer(pattern_length),
                    colorer(pattern_data_width),
                    colorer(32),
                )
            )
            ...
            self.add_memory(
                self.reader_pattern_mem.data, name="reader_pattern_data", origin=0x22000000
            )
            self.add_memory(
                self.reader_pattern_mem.addr, name="reader_pattern_addr", origin=0x23000000
            )
            self.logger.info(
                "{}: Length: {}, Data Width: {}-bit, Address width: {}-bit".format(
                    colorer("Reader BIST pattern"),
                    colorer(pattern_length),
                    colorer(pattern_data_width),
                    colorer(32),
                )
            ...
            self.submodules.writer = Writer(
                dram_wr_port, self.writer_pattern_mem, **inversion_kwargs
            )
            ...
            self.submodules.reader = Reader(
                dram_rd_port, self.reader_pattern_mem, **inversion_kwargs
            )
        ...
        if not args.no_payload_executor:
            ...
            self.logger.info(
                "{}: Length: {}, Data Width: {}-bit".format(
                    colorer("Scratchpad memory"),
                    colorer(scratchpad_depth),
    ...
    def add_memory(self, mem, *, name, origin, mode="rw"):
        ...
        self.logger.info(
            "RAM {} {} {}.".format(
                colorer(name), colorer("added", color="green"), self.bus.regions[name]
            )
        )
    ...
    def generate_sdram_phy_py_header(self, output_file):
        ...
        f.write(
            get_sdram_phy_py_header(
                self.sdram.controller.settings.phy, self.sdram.controller.settings.timing
            )
        )
        f.close()
...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def _add_common(self, *, sys_clk_freq, module):
        ...
        self.add(
            ...
            "--trace-dram-phy-dfi",
            action="store_true",
            help="Trace communication to and from PHY over DFI",
        )
        ...
        self.add(
            g, "--no-sdram-hw-test", action="store_true", help="Disable HW accelerated memory test"
        )
        self.add(
            g, "--pattern-data-size", default="1024", help="BIST pattern data memory size in bytes"
        )
        self.add(
            g, "--no-payload-executor", action="store_true", help="Disable Payload Executor module"
        )
        ...
        self.add(
            g,
            "--keep-going-on-dram-error",
            action="store_true",
            help="Continue DRAM training even if part of it fails",
        )
        self.add(
            g,
...
def get_sdram_module(name):
    ...
    if upstream is not None and local is not None:
        log.warning(
            f"Module {name} defined both in LiteDRAM and in rowhammer-tester!"
