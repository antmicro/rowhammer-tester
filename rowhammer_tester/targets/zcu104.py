...
class CRG(Module):
    ...
    def __init__(self, platform, sys_clk_freq, iodelay_clk_freq):
        ...
        self.specials += [
            ...
            Instance(
                "BUFGCE", name="main_bufgce", i_CE=1, i_I=self.cd_pll4x.clk, o_O=self.cd_sys4x.clk
            ),
        ]
...
class SoC(common.RowHammerSoC):
    def __init__(self, **kwargs):
        ...
        self.submodules.i2c = I2CMaster(
            self.platform.request("i2c"), sys_freq=self.sys_clk_freq, bus_freq=400e3, fifo_depth=192
        )
        self.add_csr("i2c")
        ...
        wb_ps = wishbone.Interface(
            adr_width=40 - 2
        )  # AXILite2Wishbone requires the same address widths
...
def main():
    parser = common.ArgumentParser(
        description="LiteX SoC on ZCU104", sys_clk_freq="125e6", module="MTA4ATF51264HZ"
    )
    g = parser.add_argument_group(title="ZCU104")
    g.add_argument(
        "--iodelay-clk-freq",
        type=float,
        help="Use given exact IODELAYCTRL reference clock frequency",
    )
    vivado_build_args(g)
