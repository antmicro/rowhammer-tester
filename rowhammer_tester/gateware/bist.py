...
class RowDataInverter(Module, AutoCSR):
    ...
    def __init__(self, addr, data_in, data_out, rowbits, row_shift):
        ...
        self.sync += [If(selector.selected, data_out.eq(~data_in)).Else(data_out.eq(data_in))]
...
class BISTModule(Module):
    ...
    def add_csrs(self):
        ...
        self._modulo = CSRStorage(
            description="When set use modulo to calculate DMA transfers address"
            " rather than bit masking"
        )
        self._count = CSRStorage(
            size=len(self.count), description="Desired number of DMA transfers"
        )
        self._done = CSRStatus(size=len(self.done), description="Number of completed DMA transfers")
        self._mem_mask = CSRStorage(
            size=len(self.mem_mask), description="DRAM address mask for DMA transfers"
        )
        self._data_mask = CSRStorage(
            size=len(self.data_mask), description="Pattern memory address mask"
        )
        self._data_div = CSRStorage(
            size=len(self.data_mask), description="Pattern memory address divisior-1"
        )
...
{common}
        ...
        fsm.act(
            ...
            If(cmd_counter >= self.count, NextState("READY")).Else(
        ...
        fsm.act(
            "INVERT_COMPUTE",
            If(wait_counter == 2, NextState("SEND")),
        ...
        fsm.act(
            ...
            If(
                ...
                If(
                    self.modulo & (mem_addr == self.data_div),
                    NextValue(mem_addr, 0),
                ).Elif(
                    self.modulo,
    ...
    def add_csrs(self):
        ...
        self.sync += [
            If(
                self.dma.sink.valid & self.dma.sink.ready,
...
The current progress can be read from the `done` CSR.
        ...
        fsm_pattern.act(
            ...
            If(counter_gen >= self.count, NextState("READY")).Else(
                NextState("WAIT_FOR_DRAM_ADDR"),
            ),
        )
        fsm_pattern.act(
        ...
        fsm_pattern.act(
            "INVERT_COMPUTE",
            If(wait_counter == 2, NextState("RD_DATA")),
            NextValue(wait_counter, wait_counter + 1),
        )
        fsm_pattern.act(
            "RD_DATA",
            If(
                ...
                If(
                    ...
                    If(self.skip_fifo, NextState("COMPUTE_MEM_ADDR")).Else(NextState("WR_ERR")),
                ).Else(NextState("COMPUTE_MEM_ADDR")),
            ),
        )
        fsm_pattern.act(
            "WR_ERR",
            error_fifo.sink.valid.eq(1),
            If(error_fifo.sink.ready | self.skip_fifo, NextState("COMPUTE_MEM_ADDR")),
        )
        ...
        fsm_error.act(
            ...
            If(
                error_reg.ready & error_fifo.source.valid,
                error_fifo.source.ready.eq(1),
                If(
                    ~self.skip_fifo,
                    NextValue(error_reg.offset, error_fifo.source.offset),
    ...
    def add_csrs(self):
        ...
        self._error_count = CSRStatus(
            size=len(self.error_count), description="Number of errors detected"
        )
        ...
        self._error_data = CSRStatus(
            size=len(self.data_port.dat_r), description="Erroneous value read from DRAM memory"
        )
        self._error_expected = CSRStatus(
            size=len(self.data_port.dat_r), description="Value expected to be read from DRAM memory"
        )
