...
from rowhammer_tester.scripts.even_row_generator import EvenRowGenerator
...
class RowListPayloadGenerator(PayloadGenerator):
    _valid_module_keys  = set(["row_generator", "read_count", "refresh", "verbose", "row_generator_config",
    ...
    def get_payload(self, *, settings, bank, payload_mem_size,
        ...
        return generate_payload_from_row_list(read_count=self.read_count,
                row_sequence=row_sequence, timings=settings.timing, bankbits=settings.geom.bankbits,
                bank=bank, payload_mem_size=payload_mem_size,
                refresh=self.refresh, verbose=self.verbose,
