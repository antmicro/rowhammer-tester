...
class HammerTolerancePayloadGenerator(PayloadGenerator):
    _valid_module_keys = set(["verbose", "row_mapping", "nr_rows", "read_count_step", "iters_per_row",
    ...
    def get_payload(self, *, settings, bank, payload_mem_size,
        ...
        row_sequence = [self.row_mapping.logical_to_physical(row_num),
                        self.row_mapping.logical_to_physical(row_num + 2)]

        return generate_payload_from_row_list(read_count=self.read_count_step * step,
                ...
                refresh=False, verbose=self.verbose,
