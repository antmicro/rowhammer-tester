...
def main() -> int:
  ...
  parser.add_argument(
      '--dram',
      type=str,
      choices=[d.name.lower() for d in DRAM],
      default=DRAM.DDR3.name.lower())
  ...
  for ip, instr in enumerate(payload.instr):
    if not dramlib.VerifyInstr(ip, instr):
      print('Illegal instruction ({}) at ip ({})'.format(
          ' '.join(str(instr).split()), ip))
      return -1
  ...
  while state.ip < len(payload.instr):
    ...
    if instr.HasField('mem'):
      if not rank.Execute(state.tick, instr.mem):
        print('Failed to execute ({}) at ip ({}) on tick ({})'.format(
      ...
      print('Illegal instruction ({}) at ip ({}) on tick ({})'.format(
          ' '.join(str(instr).split()), state.ip, state.tick))
      return -1
